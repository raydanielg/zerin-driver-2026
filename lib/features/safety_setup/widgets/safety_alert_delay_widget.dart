...
class SafetyAlertDelayWidget extends StatelessWidget {
      ...
      child: Container(
        ...
        child: Padding(
          ...
          child: Column(spacing: Dimensions.paddingSizeSmall, mainAxisSize: MainAxisSize.min, children: [
            ...
            Text(
              _typeWiseConvertTime(
                (Get.find<SplashController>().config?.safetyFeatureMinimumTripDelayTime ?? 0),
                (Get.find<SplashController>().config?.safetyFeatureMinimumTripDelayTimeType ?? ''),
              ),
  ...
  String _typeWiseConvertTime(int time, String type){
    ...
    return '${'we_notice_that_you_are_stuck_at_a_place'.tr}$calculateTime'
        ' ${(Get.find<SplashController>().config?.safetyFeatureMinimumTripDelayTimeType ?? '').tr} ${'if_you_face_any_safety_issue'.tr}';
  }
