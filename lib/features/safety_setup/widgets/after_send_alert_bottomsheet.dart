...
class _AfterSendAlertBottomSheetState extends State<AfterSendAlertBottomSheet> {
  ...
  @override
  void initState() {
    Get.find<SafetyAlertController>().getSafetyAlertDetails(Get.find<RideController>().tripDetail?.id ?? Get.find<RideController>().ongoingTrip?[0].id ?? '');
    super.initState();
  }
      ...
      return Flexible(
        child: Padding(
          ...
          child: Column(spacing: Dimensions.paddingSizeSmall,mainAxisSize: MainAxisSize.min, children: [
            ...
            if(safetyAlertController.driverSafetyDetails?.data?.comment != null)
              Container(width: Get.width,
                ...
                child: Text(
                  '${safetyAlertController.driverSafetyDetails?.data?.comment}',
            ...
            Flexible(
              child: ListView.separated(
                itemCount: safetyAlertController.driverSafetyDetails?.data?.reason?.length ?? 0,
                shrinkWrap: true,
                itemBuilder: (context,index){
                  return Container(
                    ...
                    child: Text('${safetyAlertController.driverSafetyDetails?.data?.reason?[index]}',style: textSemiBold.copyWith(fontSize: Dimensions.fontSizeLarge),textAlign: TextAlign.center),
                  );
                },
            ...
            Text(
              safetyAlertController.driverSafetyDetails?.data?.status != 'solved' ?
            ...
            if(safetyAlertController.driverSafetyDetails?.data?.status != 'solved')
            ...
            if(Get.find<SplashController>().config?.safetyFeatureEmergencyGovtNumber != null)
              InkWell(
                onTap: ()=> launchUrl(Uri.parse("tel: ${Get.find<SplashController>().config?.safetyFeatureEmergencyGovtNumber}")),
                child: Row(mainAxisSize:MainAxisSize.min, spacing: Dimensions.paddingSizeExtraSmall, children: [
                  ...
                  Text(
                    '${'call'.tr} (${Get.find<SplashController>().config?.safetyFeatureEmergencyGovtNumber})',
                    style: textRegular.copyWith(
