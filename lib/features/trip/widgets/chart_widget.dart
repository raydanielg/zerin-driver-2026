...
class _ChartWidgetState extends State<ChartWidget> {
  ...
  Widget leftTitleWidgets(double value, TitleMeta meta) {
    ...
    return SideTitleWidget(meta: meta, child: Text(value, style: textRobotoRegular.copyWith(fontSize: 10,fontWeight: FontWeight.normal,color: Color(0xff68737d))));
    ...
    touchTooltipData: LineTouchTooltipData(
      ...
      getTooltipItems: (touchedSpots) {
        return touchedSpots.map((spot) {
          if (spot.barIndex != 0) {
            return null; // vertical line points — no tooltip
          }
          return LineTooltipItem(
            PriceConverter.convertPrice(Get.context!, spot.y),
            textRegular,
          );
        }).toList();
      },
  ...
  List<LineChartBarData> lineBarsData(List<FlSpot>? spots) => [
    ...
    ...verticalLinesData(spots) ?? [],
  ];

  List<LineChartBarData>? verticalLinesData(List<FlSpot>? spots) {
